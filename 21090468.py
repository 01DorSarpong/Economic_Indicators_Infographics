...
def read_data(file):
    ...
    df2.rename(index={'1991 [YR1991]': '1991', '1992 [YR1992]': '1992',
                      '1993 [YR1993]': '1993', '1994 [YR1994]': '1994',
                      '1995 [YR1995]': '1995', '1996 [YR1996]': '1996',
                      '1997 [YR1997]': '1997', '1998 [YR1998]': '1998',
...
fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(15, 8), 
                         gridspec_kw={'hspace': 0.5}, facecolor='lightyellow')
...
# Subplot 1: Line plot showing Total Labor Force for all countries
axes[0, 0].set_title('Total Labor Force per Country',fontsize=13, fontweight='bold', color='indigo')
for country in countries:
    sns.lineplot(x=years, y=df2['Labor force, total'][country], ax=axes[0, 0],label=country)
axes[0, 0].legend(facecolor='lightyellow')
axes[0, 0].set_ylabel('Total labor force', fontsize=12,  fontweight='bold')
axes[0, 0].set_xlabel('Years', fontsize=12,  fontweight='bold')
axes[0, 0].set(facecolor='lightyellow')

# Subplot 2: Pie chart for Average Annual Inflation Growth
explode = [0.15 if country == 'China' else 0.05 for country in countries]
axes[0, 1].pie(avg_inflation, labels=countries, autopct='%1.1f%%', startangle=90,
               shadow=True, explode=explode)
axes[0, 1].set_title('Average Annual Inflation % Growth from 1991 to 2000',
...
axes[1, 0].set_title('Average Annual GDP % Growth from 1991 to 2000', 
                     fontsize=13, fontweight='bold', color='indigo')
axes[1, 0].set_ylabel('Countries', fontsize=12,  fontweight='bold')
axes[1, 0].set_xlabel('Annual % Growth', fontsize=12,  fontweight='bold')
